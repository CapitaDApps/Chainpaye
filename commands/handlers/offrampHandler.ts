...
import { whatsappBusinessService, userService } from "../../services";
...
interface OfframpSession {
  step: 'ASSET_SELECTION' | 'DEPOSIT_WAITING' | 'AMOUNT_INPUT' | 'BANK_SELECTION' | 'ACCOUNT_RESOLUTION' | 'QUOTE_GENERATION' | 'CONFIRMATION' | 'PIN_VERIFICATION';
  userId: string;
  phoneNumber: string;
 ...
 */
async function displayUserWallets(phoneNumber: string, userId: string): Promise<void> {
  try {
    ...
    for (const wallet of wallets) {
      try {
        ...
        if (wallet.chainType === 'solana') {
          balances = await crossmintService.getBalancesByChain(userId, 'solana', ['usdc', 'sol']);
        } else {
          balances = await crossmintService.getBalancesByChain(userId, wallet.chainType, ['usdc', 'usdt']);
        }
        ...
        if (nonZeroBalances.length > 0) {
          ...
          for (const balance of nonZeroBalances) {
            ...
            const usdValue = balance.usdValue ? ` (~$${balance.usdValue.toFixed(2)})` : '';
            walletsMessage += `• ${balance.token.toUpperCase()}: ${amount}${usdValue}\n`;
          }
          walletsMessage += "\n";
        }
      } catch (error) {
        console.error(`Error getting balances for ${wallet.chainType} wallet:`, error);
      }
    }
    ...
    await whatsappBusinessService.sendNormalMessage(walletsMessage, phoneNumber);
 ...
 */
export async function handleAssetSelection(phoneNumber: string, message: string): Promise<boolean> {
  try {
    ...
    const assetChainMatch = message.match(/\b(usdc|usdt)\b.*?\b(bep20|base|arbitrium|solana|hedera|apechain|lisk)\b/i);
 ...
 */
async function handleWalletCreation(phoneNumber: string, session: OfframpSession): Promise<void> {
  try {
    const chainType = crossmintService.getChainType(session.selectedChain!);
    
    // Get or create wallet
    const wallet = await crossmintService.getOrCreateWallet(session.userId, chainType);
    
    // Get current balance
    let balances: any[] = [];
    try {
      if (chainType === 'solana') {
        balances = await crossmintService.getBalancesByChain(session.userId, session.selectedChain!, ['usdc', 'sol']);
      } else {
        balances = await crossmintService.getBalancesByChain(session.userId, session.selectedChain!, ['usdc', 'usdt']);
      }
    } catch (error) {
    ...
    const assetBalance = balances.find(b => b.token.toLowerCase() === session.selectedAsset);
    ...
    const message = `🏦 *${selectedAsset.toUpperCase()} Wallet Ready*\n\n` +
 ...
 */
export async function handleAmountInput(phoneNumber: string, message: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
...
function getSupportedAssetsMessage(): string {
  return `💡 *Tell me what asset you want to deposit and its chain.*\n\n` +
 ...
 */
export async function handleBankSelection(phoneNumber: string, message: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
    if (!sessionData) return false;
 ...
 */
export async function handleAccountResolution(phoneNumber: string, message: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
    if (!sessionData) return false;
    ...
    try {
      ...
      const resolvedAccount = await dexPayService.resolveAccount(accountNumber!, session.selectedBank!.code);
      ...
      const confirmMessage = `✅ *Account Verified*\n\n` +
      ...
      await whatsappBusinessService.sendNormalMessage(confirmMessage, phoneNumber);
    ...
    console.error(`Error in handleAccountResolution for ${phoneNumber}:`, error);
    await whatsappBusinessService.sendNormalMessage(
 ...
 */
export async function handleAccountConfirmation(phoneNumber: string, message: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
    if (!sessionData) return false;
    ...
    console.error(`Error in handleAccountConfirmation for ${phoneNumber}:`, error);
    await whatsappBusinessService.sendNormalMessage(
      "❌ *Error*\n\nSomething went wrong. Please try again.",
      phoneNumber
    );
 ...
 */
async function generateQuote(phoneNumber: string, session: OfframpSession): Promise<void> {
  try {
    await whatsappBusinessService.sendNormalMessage(
    ...
    const quoteRequest = {
      ...
      receivingAddress: dexPayService.getReceivingAddress(session.selectedChain!),
    };
    ...
    try {
      if (chainType === 'solana') {
        balances = await crossmintService.getBalancesByChain(session.userId, selectedChain, ['usdc', 'sol']);
      } else {
        balances = await crossmintService.getBalancesByChain(session.userId, selectedChain, ['usdc', 'usdt']);
      }
    } catch (error) {
      console.error("Error getting balances for quote:", error);
    }

    const assetBalance = balances.find(b => b.token.toLowerCase() === selectedAsset);
    const currentBalance = assetBalance ? parseFloat(assetBalance.amount) : 0;
    
    // Calculate total required amount including all fees
    const totalFees = dexPayService.calculateFees(session.ngnAmount!, quote.rate);
    const feesInCrypto = totalFees.totalFees / quote.rate; // Convert fees to crypto amount
    const requiredAmount = quote.cryptoAmount + feesInCrypto + (quote.fees?.networkFee || 0);
 ...
 */
export async function handleTransactionConfirmation(phoneNumber: string, message: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
    if (!sessionData) return false;
    ...
    console.error(`Error in handleTransactionConfirmation for ${phoneNumber}:`, error);
    await whatsappBusinessService.sendNormalMessage(
      "❌ *Error*\n\nSomething went wrong. Please try again.",
      phoneNumber
    );
 ...
 */
export async function handlePinVerification(phoneNumber: string, message: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
    if (!sessionData) return false;
 ...
 * Execute the off-ramp transaction
 */
async function executeOfframpTransaction(phoneNumber: string, session: OfframpSession): Promise<void> {
  try {
    await whatsappBusinessService.sendNormalMessage(
    ...
    const platformFees = dexPayService.calculateFees(session.ngnAmount!, quote.rate);
    const totalCryptoAmount = quote.cryptoAmount + (platformFees.totalFees / quote.rate); // Convert fees to crypto
    ...
    try {
      const offrampResult = await dexPayService.completeOfframp(session.quote!.id);
      ...
      const selectedAsset = session.selectedAsset!;
      const successMessage = `🎉 *Off-ramp Successful!*\n\n` +
      ...
      await whatsappBusinessService.sendNormalMessage(successMessage, phoneNumber);
      ...
      console.log(`Off-ramp completed successfully for ${phoneNumber}:`, offrampResult);
    ...
    console.error(`Error executing off-ramp transaction for ${phoneNumber}:`, error);
    await whatsappBusinessService.sendNormalMessage(
 ...
 */
export async function handleDepositNotification(phoneNumber: string, asset: string, amount: number, chain: string): Promise<void> {
  try {
    // Enhanced deposit notification with better formatting
    const notificationMessage = `🎉 *Crypto Deposit Received!*\n\n` +
    ...
    await whatsappBusinessService.sendNormalMessage(notificationMessage, phoneNumber);

    // Log the deposit notification
    console.log(`Deposit notification sent to ${phoneNumber}: ${amount} ${asset} on ${chain}`);
    ...
    console.error(`Error sending deposit notification to ${phoneNumber}:`, error);
  }
}
...
export async function sendOfframpSuccessNotification(
  ...
  try {
    const successNotification = `🎉 *Off-ramp Completed Successfully!*\n\n` +
    ...
    await whatsappBusinessService.sendNormalMessage(successNotification, phoneNumber);

    // Log the success notification
    console.log(`Off-ramp success notification sent to ${phoneNumber}: ${ngnAmount} NGN`);

  } catch (error) {
    console.error(`Error sending off-ramp success notification to ${phoneNumber}:`, error);
  }
}
 ...
 */
export async function isOfframpSessionActive(phoneNumber: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
    return !!sessionData;
  } catch (error) {
 ...
 */
export async function routeOfframpMessage(phoneNumber: string, message: string): Promise<boolean> {
  try {
    const sessionData = await redisClient.get(`offramp_session:${phoneNumber}`);
    if (!sessionData) return false;

      ...
      try {
        ...
        if (validBalances.length > 0) {
          walletsWithBalances.push({
            ...
            balance: validBalances.reduce((sum, b) => sum + parseFloat(b.amount), 0)
          });
        }
      } catch (error) {
        console.error(
  ...
  try {
    ...
    try {
      ...
      const assetBalance = balances.find(
        (b) => (b.symbol?.toLowerCase() || b.token?.toLowerCase()) === selectedAsset.toLowerCase(),
      );
    ...
    console.error(`Error in handleDepositConfirmation for ${phoneNumber}:`, error);
  }
}
...
export async function handleSpendCrypto(phoneNumber: string): Promise<boolean> {
  try {
    ...
    if (!workflowState.stepData.selectedAsset || !workflowState.stepData.selectedChain) {
      await whatsappBusinessService.sendNormalMessage(
...
export async function handleAmountInput(
  ...
  try {
    ...
    const validation = validationService.validateTransactionLimits(ngnAmount, workflowState.userId);
    if (!validation.isValid) {
      await whatsappBusinessService.sendNormalMessage(
...
export async function handleBankSelection(
  ...
  try {
    ...
    if (!banksData) {
      const workflowState = await workflowController.getWorkflowState(workflowId);
      await showBankSelection(phoneNumber, workflowId, workflowState.stepData.amount);
      return true;
    }
...
export async function handleAccountConfirmation(
  ...
  try {
    ...
    const financialCalc = financialService.calculateTransactionFinancials(amount, exchangeRate);
    ...
    await whatsappBusinessService.sendNormalMessage(summaryMessage, phoneNumber);
    return true;
  } catch (error) {
    console.error(
...
export async function handlePinVerification(
  ...
  try {
    ...
    const pinValid = await authenticationService.validatePin(workflowState.userId, pin);
...
async function executeOfframpTransaction(
  ...
  try {
    ...
    if (!workflowState.stepData.bankCode || !workflowState.stepData.accountNumber) {
      await whatsappBusinessService.sendNormalMessage(
    ...
    if (result.success) {
      ...
      if (quoteStepResult.success) {
        ...
        const completionResult = await workflowController.processStep(workflowId, {
        ...
        if (completionResult.success) {
          ...
          if (result.receipt) {
            try {
              await notificationService.sendReceipt(workflowState.userId, transaction);
            } catch (receiptError) {
              console.error(`Receipt delivery error for ${workflowId}:`, receiptError);
      ...
      if (result.error?.includes("insufficient")) {
        errorMessage += "You don't have enough crypto balance for this transaction.\n\n";
        errorMessage += `Required: ${financialCalc.totalInUsd.toFixed(6)} ${workflowState.stepData.selectedAsset.toUpperCase()}\n`;
        errorMessage += "Please deposit more crypto or reduce the transaction amount.";
      } else if (result.error?.includes("network") || result.error?.includes("timeout")) {
        errorMessage += "Network connection issue. Please check your internet connection and try again.";
      } else if (result.error?.includes("rate") || result.error?.includes("expired")) {
        errorMessage += "Exchange rate has expired. Please restart the transaction for current rates.";
      } else if (result.error?.includes("bank") || result.error?.includes("account")) {
        errorMessage += "Banking service issue. Please verify your account details and try again.";
      } else {
    ...
    if (error instanceof Error) {
      if (error.message.includes("timeout")) {
        errorMessage += "The transaction timed out. Please try again with a stable internet connection.";
      } else if (error.message.includes("network")) {
        errorMessage += "Network error occurred. Please check your connection and try again.";
      } else if (error.message.includes("service")) {
        errorMessage += "One of our services is temporarily unavailable. Please try again in a few minutes.";
      } else {
...
export async function updateWorkflowForDeposit(
  ...
  try {
    console.log(`Updating workflows for deposit: ${amount} ${asset} on ${chain} for user ${userId}`);

    // Get user's active workflows that are waiting for deposit confirmation
    const activeWorkflows = await workflowController.getUserActiveWorkflows(userId);
    const waitingWorkflows = activeWorkflows.filter(workflow => 
      workflow.currentStep === OffRampStep.DEPOSIT_CONFIRMATION &&
      workflow.stepData.selectedAsset?.toLowerCase() === asset.toLowerCase() &&
      workflow.stepData.selectedChain?.toLowerCase() === chain.toLowerCase() &&
      workflow.stepData.walletAddress === walletAddress
    );

    if (waitingWorkflows.length === 0) {
      console.log(`No workflows waiting for deposit confirmation for user ${userId}`);
      return {
        success: true,
        workflowsUpdated: 0,
    ...
    for (const workflow of waitingWorkflows) {
      try {
        ...
        const stepResult = await workflowController.processStep(workflow.id, stepData);

        if (stepResult.success) {
          updatedCount++;
          console.log(`Workflow ${workflow.id} updated with deposit confirmation`);
          ...
          console.error(`Failed to update workflow ${workflow.id}:`, stepResult.error);
        }
      } catch (workflowError: any) {
        errors.push(`Workflow ${workflow.id}: ${workflowError.message}`);
        console.error(`Error updating workflow ${workflow.id}:`, workflowError.message);
      }
    }
...
async function sendWorkflowDepositNotification(
  ...
  try {
    ...
    await whatsappBusinessService.sendNormalMessage(message, phoneNumber);
    console.log(`Workflow-specific deposit notification sent to ${phoneNumber} for workflow ${workflowId}`);
  } catch (error) {
    console.error(`Error sending workflow deposit notification to ${phoneNumber}:`, error);
  }
}
...
export async function handleDepositNotification(
  ...
  try {
    console.log(`Processing deposit notification via WebhookHandler for ${phoneNumber}: ${amount} ${asset} on ${chain}`);
    ...
    try {
      ...
      if (!walletAddress) {
        console.warn(`No wallet found for user ${user.userId} on chain ${chainType}`);
        // Still proceed with notification, but log the issue
      }
    } catch (walletError) {
      console.error(`Error getting wallet address for ${phoneNumber}:`, walletError);
    ...
    try {
      const webhookResult = await (webhookHandler as any).processDepositConfirmation(webhookEvent);
      
      if (webhookResult.success) {
        workflowsUpdated = webhookResult.workflowsUpdated;
        webhookProcessingSuccess = true;
        console.log(`WebhookHandler successfully processed deposit: ${webhookResult.message}, workflows updated: ${workflowsUpdated}`);
      } else {
        console.warn(`WebhookHandler processing had issues: ${webhookResult.error}`);
        // Continue with notification even if workflow update fails
      }
    } catch (webhookError) {
      console.error(`WebhookHandler processing error for ${phoneNumber}:`, webhookError);
    ...
    let notificationMessage = `🎉 *Crypto Deposit Received!*\n\n` +
      `💰 *Amount:* ${amount.toFixed(6)} ${asset.toUpperCase()}\n` +
      `🔗 *Network:* ${chain.toUpperCase()}\n` +
      `⏰ *Time:* ${new Date().toLocaleString("en-NG", { timeZone: "Africa/Lagos" })}\n\n` +
  ...
  try {
    ...
    switch (workflowState.currentStep) {
      ...
      case OffRampStep.BALANCE_VALIDATION:
        try {
          ...
          console.error(`Account confirmation error for ${phoneNumber}:`, error);
          await whatsappBusinessService.sendNormalMessage(
    ...
    if (error instanceof Error) {
      if (error.message.includes("workflow")) {
        errorMessage += "There was an issue with your transaction workflow. Please start a new transaction.";
      } else if (error.message.includes("network")) {
...
async function handleWorkflowError(
  ...
  if (error.message.includes("insufficient")) {
    errorMessage += "You don't have enough balance for this transaction. Please deposit more crypto or reduce the amount.";
  } else if (error.message.includes("expired")) {
    errorMessage += "Your session has expired. Please start a new transaction.";
  } else if (error.message.includes("invalid")) {
    errorMessage += "Invalid input provided. Please check your information and try again.";
...
async function getWorkflowId(phoneNumber: string): Promise<string | null> {
  try {
    ...
    const storedWorkflowId = await redisClient.get(`offramp_workflow:${phoneNumber}`);
    if (storedWorkflowId) {

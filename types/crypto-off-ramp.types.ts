...
export type SupportedChain = 'bep20' | 'base' | 'arbitrum' | 'solana' | 'hedera' | 'apechain' | 'lisk';
...
export interface INotificationService {
  ...
  sendCompletionNotification?(userId: string, transaction: OffRampTransaction): Promise<void>;
}
...
export interface ITransactionManager {
  processTransaction(transaction: OffRampTransaction): Promise<TransactionResult>;
  executeCryptoTransfer(transferRequest: TransferRequest): Promise<TransferResult>;
...
export interface IWalletManager {
  ...
  validateWalletBalance(walletAddress: string, requiredAmount: number): Promise<boolean>;
}
...
export interface IBankingManager {
  ...
  resolveBank(bankCode: string, accountNumber: string): Promise<BankResolution>;
  getCurrentRates(asset: string, chain: string): Promise<ExchangeRate>;
  validateBankDetails(bankCode: string, accountNumber: string): Promise<ValidationResult>;
}

...
export class CrossmintService {
  ...
  constructor() {
    ...
    this.baseUrl = process.env.CROSSMINT_BASE_URL || "https://staging.crossmint.com/api/2025-06-09";
  ...
  async listWallets(userId: string): Promise<CrossmintWallet[]> {
    try {
      const response = await axios.get(`${this.baseUrl}/wallets/userId:${userId}:evm/balance?tokens=usdc`, {
        headers: {
      ...
      logger.error(`Error listing wallets for user ${userId}:`, error.response?.data || error.message);
   ...
   */
  async createWallet(userId: string, chainType: string): Promise<CrossmintWallet> {
    try {
      ...
      const response = await axios.post(`${this.baseUrl}/wallets`, requestBody, {
        headers: {
          "X-API-KEY": this.apiKey,
      ...
      logger.info(`Created ${chainType} wallet for user ${userId}: ${response.data.address}`);
      return response.data;
    } catch (error: any) {
      logger.error(`Error creating ${chainType} wallet for user ${userId}:`, error.response?.data || error.message);
   ...
   */
  async getBalancesByChain(userId: string, chain: string, tokens: string[] = ["usdc", "usdt"]): Promise<CrossmintBalance[]> {
    const chainId = this.getChainIdentifier(chain);
    return this.getWalletBalances(userId, chainId, tokens);
  }
  async getWalletBalances(userId: string, chain: string, tokens: string[] = ["usdc", "usdt"]): Promise<CrossmintBalance[]> {
    try {
      const response = await axios.get(`${this.baseUrl}/wallets/userId:${userId}:${chain}/balances`, {
        headers: {
          "X-API-KEY": this.apiKey,
        },
        params: {
      ...
      logger.info(`Retrieved ${response.data.length} balances for user ${userId} on ${chain}`);
      return response.data;
    } catch (error: any) {
      logger.error(`Error getting ${chain} balances for user ${userId}:`, error.response?.data || error.message);
      throw new Error(`Failed to get ${chain} balances: ${error.response?.data?.message || error.message}`);
    }
  }
   ...
   */
  async getSolanaBalances(userId: string, tokens: string[] = ["usdc", "sol"]): Promise<CrossmintBalance[]> {
   ...
   */
  async getEvmBalances(userId: string, tokens: string[] = ["usdc", "usdt"]): Promise<CrossmintBalance[]> {
   ...
   */
  async getWalletByChain(userId: string, chainType: string): Promise<CrossmintWallet | null> {
    try {
      const wallets = await this.listWallets(userId);
      return wallets.find(wallet => wallet.chainType === chainType) || null;
    } catch (error) {
      logger.error(`Error getting ${chainType} wallet for user ${userId}:`, error);
      return null;
    }
  }
   ...
   */
  async getOrCreateWallet(userId: string, chainType: string): Promise<CrossmintWallet> {
    try {
      ...
      logger.error(`Error getting or creating ${chainType} wallet for user ${userId}:`, error);
      throw error;
    }
  }
  ...
  async transferTokens(
    ...
    try {
      ...
      if (!wallet) {
        throw new Error(`No wallet found for user ${userId} on chain ${chainType}`);
      }
      ...
      logger.info(`Transferred ${amount} ${token} from wallet ${wallet.address} to ${toAddress}`);
      return response.data;
    } catch (error: any) {
      logger.error(`Error transferring tokens for user ${userId}:`, error.response?.data || error.message);
      throw new Error(`Failed to transfer tokens: ${error.response?.data?.message || error.message}`);
    }
  }
  ...
  getTokenChainIdentifier(chainType: string): string {
    ...
    return tokenChainMapping[chainType.toLowerCase()] || chainType.toLowerCase();
  }

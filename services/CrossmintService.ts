...
export class CrossmintService implements ICrossmintService, IWalletManager {
    ...
    try {
      ...
      for (const wallet of crossmintWallets) {
        try {
          ...
          const balances = await this.getWalletBalances(userId, wallet.chainType);
          ...
          logger.warn(`Failed to get balances for wallet ${wallet.address}:`, balanceError.message);
      ...
      logger.info(`Retrieved ${walletInfos.length} wallet infos for user ${userId}`);
      return walletInfos;
    } catch (error: any) {
   ...
   */
  async getWalletBalances(userId: string, chainType: string): Promise<Balance[]> {
    try {
      // Validate inputs
      if (!userId || typeof userId !== 'string') {
        throw new Error('Valid userId is required');
      ...
      const balances: Balance[] = crossmintBalances.map(balance => {
        ...
        if (!asset) {
          logger.warn(`Balance missing asset information for user ${userId} on ${chainType}:`, balance);
        }
      ...
      logger.info(`Retrieved ${balances.length} balances for user ${userId} on ${chainType}:`, 
        balances.map(b => `${b.amount} ${b.asset} ($${b.usdValue})`));
      return balances;
    } catch (error: any) {
      logger.error(`Error getting wallet balances for ${userId} on ${chainType}:`, error.message);
  ...
  async createWallet(userId: string, chainType: string): Promise<WalletInfo> {
    try {
      ...
      const supportedChainTypes = ['evm', 'solana', 'bsc', 'base', 'arbitrum', 'hedera', 'apechain', 'lisk'];
      if (!supportedChainTypes.includes(chainType.toLowerCase())) {
        throw new Error(`Unsupported chainType: ${chainType}. Supported types: ${supportedChainTypes.join(', ')}`);
      }

      const crossmintWallet = await this.createWalletInternal(userId, chainType);
      ...
      logger.error(`Error creating wallet for ${userId} on ${chainType}:`, error.message);
   ...
   */
  async transferTokens(transferRequest: TransferRequest): Promise<TransferResult> {
    try {
      ...
      const userId = await this.extractUserIdFromWalletAddress(transferRequest.walletAddress);
      ...
      if (!wallet || wallet.address !== transferRequest.walletAddress) {
        logger.error(`Wallet verification failed for address ${transferRequest.walletAddress}:`, {
          walletFound: !!wallet,
      ...
      return {
        success: true,
        transactionId: result.id || result.transactionId || this.generateRandomString(16),
      };
    } catch (error: any) {
   ...
   */
  async ensureWalletExists(userId: string, chainType: string): Promise<WalletInfo> {
    try {
      ...
      const supportedChainTypes = ['evm', 'solana', 'bsc', 'base', 'arbitrum', 'hedera', 'apechain', 'lisk'];
      if (!supportedChainTypes.includes(chainType.toLowerCase())) {
        throw new Error(`Unsupported chainType for off-ramp: ${chainType}. Supported types: ${supportedChainTypes.join(', ')}`);
      }
      ...
      if (existingWallet) {
        ...
        logger.info(`Reusing existing wallet for user ${userId} on ${chainType}:`, {
          address: walletInfo.address,
          balanceCount: balances.length,
      ...
      logger.info(`Creating new wallet for user ${userId} on ${chainType} (no existing wallet found)`);
      const newWalletInfo = await this.createWallet(userId, chainType);
      
      logger.info(`Successfully ensured wallet exists for user ${userId} on ${chainType}:`, {
      ...
      logger.error(`Error ensuring wallet exists for ${userId} on ${chainType}:`, {
        error: error.message,
        chainType,
        userId
      });
   ...
   */
  async validateWalletBalance(walletAddress: string, requiredAmount: number): Promise<boolean> {
    try {
      // Enhanced input validation
      if (!walletAddress || typeof walletAddress !== 'string') {
        logger.warn(`Invalid wallet address for balance validation: ${walletAddress}`);
        return false;
      }
      
      if (typeof requiredAmount !== 'number' || requiredAmount < 0) {
        logger.warn(`Invalid required amount for balance validation: ${requiredAmount}`);
        return false;
      }
      ...
      if (!targetWallet) {
        logger.warn(`Wallet not found during balance validation: ${walletAddress}`);
        return false;
      }
   ...
   */
  async getWalletInfoForWorkflow(userId: string, chainType?: string): Promise<any[]> {
    try {
      logger.info(`Getting wallet info for workflow - user: ${userId}, chainType: ${chainType}`);
      
      if (chainType) {
        // Get specific wallet for the chain
        const walletInfo = await this.ensureWalletExists(userId, chainType);
        const totalBalance = walletInfo.balances.reduce((sum, b) => sum + (b.usdValue || 0), 0);
        ...
        return wallets.map(wallet => ({
          address: wallet.address,
          chainType: wallet.chainType,
          balance: wallet.balances.reduce((sum, b) => sum + (b.usdValue || 0), 0),
      ...
      throw new Error(`Failed to get wallet information for workflow: ${error.message}`);
    }
  }
   ...
   */
  async getWalletBalanceForAsset(userId: string, chainType: string, asset: string): Promise<number> {
    try {
      const balances = await this.getWalletBalances(userId, chainType);
      const chain = this.mapChainTypeToChain(chainType);
   ...
   */
  private validateTransferRequest(request: TransferRequest): { isValid: boolean; error?: string } {
    ...
    if (!request.token.includes(':')) {
      return { isValid: false, error: 'Token must be in format chain:symbol (e.g., solana:usdc)' };
    }

    const [chain, symbol] = request.token.split(':');
    if (!chain || !symbol) {
      return { isValid: false, error: 'Invalid token format. Expected chain:symbol' };
    }

    // Validate supported asset-chain combinations for off-ramp
    if (!this.isAssetSupported(symbol, chain)) {
      return { isValid: false, error: `${symbol.toUpperCase()} is not supported on ${chain}` };
    }
    ...
    if (amount < 0.000001) {
      return { isValid: false, error: 'Amount too small. Minimum transfer amount is 0.000001' };
    }

    if (amount > 1000000) {
      return { isValid: false, error: 'Amount too large. Maximum transfer amount is 1,000,000' };
    }

    if (!request.idempotencyKey || typeof request.idempotencyKey !== 'string') {
      return { isValid: false, error: 'Valid idempotency key is required for off-ramp transfers' };
    }

    // Validate idempotency key format
    if (request.idempotencyKey.length < 10 || request.idempotencyKey.length > 64) {
      return { isValid: false, error: 'Idempotency key must be between 10 and 64 characters' };
    }
  ...
  private async executeTransferWithIdempotency(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const currentIdempotencyKey = attempt === 1 
          ? idempotencyKey 
        ...
        if (error.response?.status === 409) {
          ...
          throw new Error(`Transfer with idempotency key ${idempotencyKey} already processed`);
        }
  ...
  private isRetryableError(error: any): boolean {
    ...
    if (status >= 500 || status === 429 || message.includes('timeout') || message.includes('network')) {
      return true;
    }
   ...
   */
  async validateTransferStatus(transactionId: string, expectedAmount: string, expectedToken: string): Promise<{
    isValid: boolean;
    status: string;
    actualAmount?: string;
    actualToken?: string;
  ...
  async getTransferDetails(transactionId: string): Promise<{
    ...
    try {
      logger.info(`Getting transfer details for off-ramp workflow:`, { transactionId });
  ...
  private translateTransferError(error: any): string {
    ...
    if (errorMessage.includes('invalid recipient') || errorMessage.includes('invalid address')) {
      return 'Invalid recipient address. Please verify the destination address and try again.';
    }
    
    if (errorMessage.includes('invalid token') || errorMessage.includes('unsupported token')) {
    ...
    if (errorMessage.includes('wallet not found') || errorMessage.includes('wallet does not exist')) {
      return 'Wallet not found. Please ensure your wallet is properly set up and try again.';
    }
    
    if (errorMessage.includes('unauthorized') || errorMessage.includes('permission') || errorCode === 401 || errorCode === 403) {
      return 'Transfer authorization failed. Please verify your account permissions.';
    }
    
    if (errorMessage.includes('amount') && (errorMessage.includes('minimum') || errorMessage.includes('maximum'))) {
    ...
    logger.error('Unhandled transfer error:', { errorMessage, errorCode, error });
   ...
   */
  private async extractUserIdFromWalletAddress(walletAddress: string): Promise<string> {
    try {
      // Check our in-memory mapping first
      const cachedUserId = this.walletUserMappings.get(walletAddress);
      if (cachedUserId) {
        logger.info(`Found cached userId for wallet ${walletAddress}: ${cachedUserId}`);
        return cachedUserId;
      }
      ...
      if (walletAddress.startsWith('0x') || walletAddress.length > 32) {
        ...
        this.walletUserMappings.set(walletAddress, userId);
        logger.info(`Created deterministic mapping for wallet ${walletAddress} to user ${userId}`);
        return userId;
      }
      
      // Fallback for unknown wallet patterns
      logger.warn(`Could not extract userId from wallet address: ${walletAddress}`);
      throw new Error(`Unable to determine wallet owner for address: ${walletAddress}`);
    } catch (error: any) {
      logger.error(`Error extracting userId from wallet address ${walletAddress}:`, error.message);
   ...
   */
  generateIdempotencyKey(userId: string, walletAddress: string, token: string, amount: string, recipient: string): string {
    ...
    ).toString('base64').replace(/[+/=]/g, '').substring(0, 16);
   ...
   */
  private registerWalletUserMapping(walletAddress: string, userId: string): void {
    this.walletUserMappings.set(walletAddress, userId);
    logger.info(`Registered wallet-user mapping for off-ramp workflow: ${walletAddress} -> ${userId}`);
   ...
   */
  async getTransferMetrics(userId: string, timeRange?: { start: Date; end: Date }): Promise<{
    ...
    try {
      ...
      logger.error(`Error getting transfer metrics for ${userId}:`, error.message);
   ...
   */
  async validateTransferPrerequisites(transferRequest: TransferRequest): Promise<{
    isValid: boolean;
    checks: {
    ...
    try {
      ...
      try {
        ...
        } else if (error.message.includes('API key') || error.message.includes('ECONNRESET')) {
          warnings.push('Could not verify wallet existence due to API configuration');
        } else {
      ...
      try {
        const amount = parseFloat(transferRequest.amount);
        const balanceCheckPromise = this.validateWalletBalance(transferRequest.walletAddress, amount);
        const timeoutPromise = new Promise<boolean>((_, reject) => 
          setTimeout(() => reject(new Error('Balance check timeout')), 3000)
        );
        
        sufficientBalance = await Promise.race([balanceCheckPromise, timeoutPromise]);
      } catch (error: any) {
        if (error.message.includes('timeout')) {
      ...
      const validRecipient = this.isValidWalletAddress(transferRequest.recipient);
      if (!validRecipient) {
  ...
  async estimateTransferFees(transferRequest: TransferRequest): Promise<{
    ...
    try {
      ...
      if (!chain || !symbol) {
        throw new Error(`Invalid token format: ${transferRequest.token}. Expected format: chain:symbol`);
      }
      
      const amount = parseFloat(transferRequest.amount);

      logger.info(`Estimating transfer fees for ${amount} ${symbol} on ${chain}`);
   ...
   */
  async cancelTransfer(transactionId: string, reason: string): Promise<{
    success: boolean;
    status: 'cancelled' | 'too_late' | 'not_found';
    message: string;
  }> {
    try {
      logger.info(`Attempting to cancel transfer ${transactionId}:`, { reason });
      ...
      if (transactionId.includes('confirmed')) {
        return {
          success: false,
          status: 'too_late',
          message: 'Transfer has already been confirmed and cannot be cancelled'
        };
      }
      ...
      logger.error(`Error cancelling transfer ${transactionId}:`, error.message);
      return {
        success: false,
        status: 'not_found',

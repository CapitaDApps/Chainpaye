...
export class ValidationService implements IValidationService {
  ...
  private static readonly SUPPORTED_COMBINATIONS: Record<SupportedAsset, SupportedChain[]> = {
  ...
  validateAssetChain(asset: string, chain: string): ValidationResult {
    ...
    if (!normalizedAsset || !ValidationService.SUPPORTED_COMBINATIONS[normalizedAsset]) {
      errors.push(`Unsupported asset: ${asset}. Supported assets are: ${Object.keys(ValidationService.SUPPORTED_COMBINATIONS).join(', ')}`);
    }

    // Validate chain is supported for the asset
    if (normalizedAsset && ValidationService.SUPPORTED_COMBINATIONS[normalizedAsset]) {
      const supportedChains = ValidationService.SUPPORTED_COMBINATIONS[normalizedAsset];
      if (!normalizedChain || !supportedChains.includes(normalizedChain)) {
        errors.push(`Unsupported chain for ${normalizedAsset}: ${chain}. Supported chains for ${normalizedAsset} are: ${supportedChains.join(', ')}`);
      }
    }
   ...
   */
  validateBankDetails(bankCode: string, accountNumber: string): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (!accountNumber || typeof accountNumber !== 'string') {
      ...
      if (/^(\d)\1{9}$/.test(cleanAccountNumber)) {
        warnings.push('Account number appears to have repeating digits - please verify');
      }
    }
   ...
   */
  validateSufficientBalance(walletBalance: number, requiredAmount: number): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (walletBalance < ValidationService.MIN_BALANCE_USD) {
      errors.push(`Wallet balance is below minimum threshold of $${ValidationService.MIN_BALANCE_USD}`);
    }
    ...
    if (remainingBalance > 0 && remainingBalance < ValidationService.MIN_BALANCE_USD) {
      warnings.push(`Transaction will leave a very low balance of $${remainingBalance.toFixed(6)}`);
    }
  ...
  validateTransactionLimits(amount: number, userId: string): ValidationResult {
    ...
    if (amount < ValidationService.MIN_TRANSACTION_USD) {
      errors.push(`Transaction amount must be at least $${ValidationService.MIN_TRANSACTION_USD}`);
    }

    // Check maximum transaction amount
    if (amount > ValidationService.MAX_TRANSACTION_USD) {
      errors.push(`Transaction amount cannot exceed $${ValidationService.MAX_TRANSACTION_USD}`);
    }
  ...
  validateSpendForm(formData: {
    ...
    const missingFields = requiredFields.filter(field => 
      ...
      (typeof formData[field as keyof typeof formData] === 'string' && 
       (formData[field as keyof typeof formData] as string).trim().length === 0)
    );

    if (missingFields.length > 0) {
      errors.push(`Missing required fields: ${missingFields.join(', ')}. All fields must be completed before proceeding.`);
    }

    // Validate individual fields if present
    if (formData.bankCode && formData.accountNumber) {
      const bankValidation = this.validateBankDetails(formData.bankCode, formData.accountNumber);
      errors.push(...bankValidation.errors);
      warnings.push(...(bankValidation.warnings || []));
    }

    if (formData.amount !== undefined) {
      const amountValidation = this.validateTransactionLimits(formData.amount, 'temp-user');
    ...
    if (formData.network) {
      const supportedNetworks = Object.values(ValidationService.SUPPORTED_COMBINATIONS).flat();
  ...
  validatePinFormat(pin: string): ValidationResult {
    ...
    if (!pin || typeof pin !== 'string') {
      ...
      // Check for weak PINs
      if (/^(\d)\1{3,5}$/.test(pin)) {
        warnings.push('PIN should not use repeating digits for better security');
      }

      if (['1234', '0000', '1111', '2222', '3333', '4444', '5555', '6666', '7777', '8888', '9999'].includes(pin)) {
        warnings.push('PIN appears to be a common pattern - consider using a more secure PIN');
      }
    }
   ...
   */
  validateWalletAddress(address: string, chain: SupportedChain): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    switch (chain) {
      ...
      default:
        warnings.push(`Address format validation not implemented for chain: ${chain}`);
    }
  ...
  validateCompleteTransaction(transactionData: {
    ...
    const assetChainValidation = this.validateAssetChain(transactionData.asset, transactionData.chain);
    errors.push(...assetChainValidation.errors);
    warnings.push(...(assetChainValidation.warnings || []));

    // Validate bank details
    const bankValidation = this.validateBankDetails(transactionData.bankCode, transactionData.accountNumber);
    errors.push(...bankValidation.errors);
    warnings.push(...(bankValidation.warnings || []));

    // Validate transaction limits
    const limitsValidation = this.validateTransactionLimits(transactionData.amount, transactionData.userId);
    errors.push(...limitsValidation.errors);
    warnings.push(...(limitsValidation.warnings || []));

    // Validate sufficient balance
    const balanceValidation = this.validateSufficientBalance(transactionData.walletBalance, transactionData.amount);

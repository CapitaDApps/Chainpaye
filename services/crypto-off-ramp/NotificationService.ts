...
export class NotificationService implements INotificationService {
   ...
   */
  async sendReceipt(userId: string, transaction: OffRampTransaction): Promise<void> {
    try {
      this.log(`Sending receipt for transaction ${transaction.id} to user ${userId}`);
      ...
      const deliveryResult = await this.deliverReceipt(userId, transaction, receipt);
      
      if (!deliveryResult.primaryChannelSuccessful) {
        throw new Error(`Failed to deliver receipt through primary channel for transaction ${transaction.id}`);
      }

      this.log(`Receipt delivered successfully for transaction ${transaction.id}`);
    } catch (error) {
      this.log(`Error sending receipt for transaction ${transaction.id}: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  async sendStatusUpdate(userId: string, message: string): Promise<void> {
    try {
      ...
      this.log(`Error sending status update to user ${userId}: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  async sendErrorNotification(userId: string, error: string): Promise<void> {
    try {
      ...
      if (this.config.enableWhatsApp) {
        await this.sendWhatsAppMessage(userId, `❌ Transaction Error: ${userFriendlyError}`);
      }

      this.log(`Error notification sent successfully to user ${userId}`);
    } catch (notificationError) {
      this.log(`Error sending error notification to user ${userId}: ${(notificationError as Error).message}`);
   ...
   * @returns Promise<void>
   */
  async sendCompletionNotification(userId: string, transaction: OffRampTransaction): Promise<void> {
    try {
      this.log(`Sending completion notification for transaction ${transaction.id} to user ${userId}`);
      ...
      this.log(`Completion notification sent successfully for transaction ${transaction.id}`);
    } catch (error) {
      this.log(`Error sending completion notification for transaction ${transaction.id}: ${(error as Error).message}`);
      throw error;
    }
  }
  ...
  private async deliverReceipt(
    ...
    for (const channel of channels) {
      try {
        this.log(`Attempting receipt delivery via ${channel} for transaction ${transaction.id}`);
        
        const result = await this.deliverReceiptViaChannel(userId, transaction, receipt, channel);
        deliveryResults.push(result);
        
        if (result.success) {
          this.log(`Receipt delivered successfully via ${channel} for transaction ${transaction.id}`);
        } else {
          this.log(`Receipt delivery failed via ${channel} for transaction ${transaction.id}: ${result.error}`);
        }
      } catch (error) {
        this.log(`Error delivering receipt via ${channel}: ${(error as Error).message}`);
    ...
    const primaryChannelResult = deliveryResults.find(r => r.channel === channels[0]);
  ...
  private async deliverReceiptViaChannel(
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        this.log(`Receipt delivery attempt ${attempt}/${this.config.retryAttempts} via ${channel}`);
        ...
        this.log(`Receipt delivery attempt ${attempt} failed via ${channel}: ${lastError.message}`);
    ...
    return {
      success: false,
      channel,
      error: lastError?.message || `Receipt delivery failed after ${this.config.retryAttempts} attempts`
    };
  }
  ...
  private async deliverReceiptViaEmail(
    ...
    this.log(`Email delivery not yet implemented for transaction ${transaction.id}`);
  ...
  private async deliverReceiptViaSMS(
    ...
    this.log(`SMS delivery not yet implemented for transaction ${transaction.id}`);
  ...
  private async deliverReceiptViaInApp(
    ...
    this.log(`In-app delivery not yet implemented for transaction ${transaction.id}`);
   ...
   */
  private async sendWhatsAppMessage(userId: string, message: string): Promise<void> {
    const userPhoneNumber = await this.getUserPhoneNumber(userId);
   ...
   * Requirements: 12.3
   */
  private generateReceiptFromTransaction(transaction: OffRampTransaction): TransactionReceipt {
    return {
      transactionId: transaction.id,
  ...
  private formatErrorForUser(error: string): string {
    ...
    const errorMappings: Record<string, string> = {
      'insufficient funds': 'You do not have enough balance for this transaction',
      'invalid pin': 'The PIN you entered is incorrect',
      'network error': 'There was a connection problem. Please try again',
      'api error': 'Our services are temporarily unavailable. Please try again later',
      'timeout': 'The transaction took too long to process. Please try again',
      'expired': 'The transaction session has expired. Please start a new transaction'
    };
  ...
  private async getUserPhoneNumber(userId: string): Promise<string | null> {
    ...
    this.log(`Getting phone number for user ${userId} (placeholder implementation)`);
  ...
  private log(message: string): void {
    console.log(`[NotificationService] ${new Date().toISOString()}: ${message}`);
  }
}

...
export class WorkflowController implements IWorkflowController {
   ...
   */
  private async executeStep(workflow: WorkflowState, stepData: any): Promise<StepResult> {
   ...
   */
  private async executeDisplayWallets(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      ...
      const validWallets = stepData.wallets.filter((wallet: any) => 
   ...
   */
  private async executeRequestAssetChain(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      // Display the required message as per requirement 3.1
      if (!stepData.asset || !stepData.chain) {
        return {
          success: false,
          error: 'Tell me what asset you want to deposit and its chain. Example: \'USDC on Solana\'',
          data: {
            promptMessage: 'Tell me what asset you want to deposit and its chain. Example: \'USDC on Solana\''
          }
        };
      }
      ...
      if (!supportedCombinations[asset as keyof typeof supportedCombinations].includes(chain)) {
        return {
          success: false,
   ...
   */
  private async executeWalletCreation(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      ...
      if (existingWallet && existingWallet.chainType === selectedChain) {
        ...
        const newWalletAddress = stepData.walletAddress || `new-wallet-${selectedChain}-${Date.now()}`;
   ...
   */
  private async executeDepositConfirmation(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      ...
      if (stepData.depositFailed === true) {
        return {
          success: false,
          error: 'Deposit failed or timed out. Please try again or contact support.',
          data: {
      ...
      return {
        success: false,
        error: 'Waiting for deposit confirmation. Please ensure you have sent the crypto to the provided address.',
        data: {
          depositPending: true,
   ...
   */
  private async executeSpendForm(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
   ...
   */
  private async executeBankResolution(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      ...
      if (!accountName) {
        return {
          success: false,
          error: 'Failed to resolve bank account. Please verify your account number and bank selection.'
        };
      }
   ...
   */
  private async executeBalanceValidation(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      const amount = workflow.stepData.amount;
      ...
      if (!amount || !exchangeRate || walletBalance === undefined) {
        return {
          success: false,
          error: 'Missing balance validation parameters. Please ensure wallet balance is available.'
        };
      }
   ...
   */
  private async executePinConfirmation(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      ...
      if (!stepData.pin) {
        return {
          ...
          data: {
            ...
            securityNotice: 'Your PIN protects your funds from unauthorized access'
          }
        };
      }
      ...
      if (pinValid === false) {
        ...
        await this.handleStepFailure(workflow.id, new Error('Incorrect PIN provided'));
   ...
   */
  private async executeCryptoTransfer(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      ...
      if (transferSuccess === false) {
        ...
        await this.handleStepFailure(workflow.id, new Error('Crypto transfer failed'));
        
        return {
          success: false,
          error: 'Crypto transfer failed. Your funds are safe. Please try again.',
          data: {
            transferFailed: true,
            fundsSecure: true,
            dexpayCallsPrevented: true,
            retryAllowed: true,
            failureReason: stepData.failureReason || 'Transfer could not be completed'
          }
        };
      }
   ...
   */
  private async executeQuoteCreation(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
   ...
   */
  private async executeQuoteFinalization(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
      ...
      if (quoteDetails && quoteDetails.expiresAt && new Date() > new Date(quoteDetails.expiresAt)) {
        return {
          success: false,
   ...
   */
  private async executeCompletion(workflow: WorkflowState, stepData: any): Promise<StepResult> {
    try {
   ...
   */
  private isValidStepTransition(currentStep: OffRampStep, nextStep: OffRampStep): boolean {

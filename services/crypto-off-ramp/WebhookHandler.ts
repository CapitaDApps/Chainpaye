...
export class WebhookHandler {
  ...
  private async processDepositConfirmation(
    ...
    try {
      ...
      const assetChainValidation = this.validationService.validateAssetChain(
        asset,
        chainType,
      );
      ...
      const activeWorkflows =
        await this.workflowController.getUserActiveWorkflows(userId);
      ...
      for (const workflow of waitingWorkflows) {
        try {
          ...
          const walletBalance =
            await this.crossmintService.getWalletBalanceForAsset(
              userId,
              chainType,
              asset,
            );
          ...
          if (stepResult.success) {
            ...
            try {
              ...
              const { updateWorkflowForDeposit } =
                await import("../../commands/handlers/offrampHandler");
              ...
              logger.warn(
                `Direct handler integration failed (non-critical):`,
  ...
  private verifyWebhookSignature(req: Request): boolean {
    try {
      ...
      const signature =
        req.get("x-crossmint-signature") || req.get("x-hub-signature-256");
      if (!signature) {
   ...
   */
  private validateWebhookEvent(event: any): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (!event.type) {
      ...
      errors.push(
        `Invalid event type: ${event.type}. Expected: wallet.deposit`,
      );
    }
  ...
  async handleTestWebhook(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!userId || !asset || !amount || !chain || !walletAddress) {
        res.status(400).json({
          error:
            "Missing required fields: userId, asset, amount, chain, walletAddress",
        });
        return;
      }

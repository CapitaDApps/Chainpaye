...
export class TransactionManager implements ITransactionManager {
  ...
  private readonly transactionContexts: Map<string, TransactionContext> = new Map();
   ...
   */
  async processTransaction(transaction: OffRampTransaction): Promise<TransactionResult> {
    ...
    try {
      ...
      await this.updateTransactionStatus(transactionId, TransactionStatus.PIN_CONFIRMED);

      // Step 1: Execute crypto transfer first and exclusively (Requirement 10.1)
      this.log(`Step 1: Executing crypto transfer for transaction ${transactionId}`);
      ...
      if (!cryptoResult.success) {
        ...
        this.log(`Crypto transfer failed for ${transactionId}, aborting transaction`);
      ...
      await this.updateTransactionStatus(transactionId, TransactionStatus.CRYPTO_TRANSFERRED);
      ...
      this.log(`Step 2: Creating DexPay quote for transaction ${transactionId}`);
      ...
      await this.updateTransactionStatus(transactionId, TransactionStatus.QUOTE_CREATED);
      this.log(`Quote created successfully for ${transactionId}: ${quoteResult.quoteId}`);
      ...
      if (this.notificationService) {
        try {
          await this.notificationService.sendCompletionNotification?.(transaction.userId, transaction);
          await this.notificationService.sendReceipt(transaction.userId, transaction);
          this.log(`Completion notification and receipt sent for transaction ${transactionId}`);
        } catch (notificationError) {
          this.log(`Warning: Failed to send notifications for transaction ${transactionId}: ${(notificationError as Error).message}`);
      ...
      this.log(`Unexpected error processing transaction ${transactionId}: ${(error as Error).message}`);
      await this.updateTransactionStatus(transactionId, TransactionStatus.FAILED);
   ...
   */
  async executeCryptoTransfer(transferRequest: TransferRequest): Promise<TransferResult> {
    try {
      this.log(`Executing crypto transfer: ${transferRequest.amount} ${transferRequest.token}`);
      ...
      for (let attempt = 1; attempt <= this.config.maxRetryAttempts; attempt++) {
        try {
          this.log(`Transfer attempt ${attempt}/${this.config.maxRetryAttempts}`);
          
          const result = await this.crossmintService.transferTokens(transferRequest);
          
          if (result.success) {
            this.log(`Crypto transfer successful on attempt ${attempt}: ${result.transactionId}`);
            return result;
          } else {
          ...
          this.log(`Transfer attempt ${attempt} threw error: ${lastError.message}`);
        }
      ...
      return {
        success: false,
        error: lastError?.message || 'Crypto transfer failed after all retry attempts'
      };
  ...
  async createDexpayQuote(quoteRequest: QuoteRequest): Promise<Quote> {
    try {
      this.log(`Creating DexPay quote for ${quoteRequest.asset} on ${quoteRequest.chain}`);
      ...
      for (let attempt = 1; attempt <= this.config.maxRetryAttempts; attempt++) {
        try {
          this.log(`Quote creation attempt ${attempt}/${this.config.maxRetryAttempts}`);
          
          const quote = await this.dexPayService.createQuote(quoteRequest);
          
          this.log(`Quote created successfully on attempt ${attempt}: ${quote.id}`);
          return quote;
        } catch (error) {
          lastError = error as Error;
          this.log(`Quote creation attempt ${attempt} failed: ${lastError.message}`);
      ...
      throw lastError || new Error('Quote creation failed after all retry attempts');
  ...
  async finalizeTransaction(transactionId: string): Promise<TransactionResult> {
    try {
      ...
      this.log(`Finalizing transaction ${transactionId} with quote ${transaction.dexpayQuoteId}`);

      const result = await this.dexPayService.finalizeQuote(transaction.dexpayQuoteId);
      ...
      if (result.success) {
        ...
        if (result.expired && result.requiresRegeneration) {
          this.log(`Quote expired for transaction ${transactionId}, regeneration required`);
          return {
            success: false,
            transactionId,
            error: 'Quote has expired. Please restart the transaction for current rates.',
            status: TransactionStatus.FAILED
          };
        }
   ...
   */
  getTransactionDetails(transactionId: string): { transaction: OffRampTransaction; context: TransactionContext } | null {
    const transaction = this.transactions.get(transactionId);
  ...
  async cancelTransaction(transactionId: string): Promise<boolean> {
    try {
      ...
      await this.updateTransactionStatus(transactionId, TransactionStatus.CANCELLED);
      this.log(`Transaction ${transactionId} cancelled`);
      return true;
    } catch (error) {
      this.log(`Error cancelling transaction ${transactionId}: ${(error as Error).message}`);
      return false;
    }
  }
   ...
   */
  private async executeCryptoTransferStep(transaction: OffRampTransaction): Promise<{ success: boolean; transactionId?: string; error?: string }> {
    ...
    try {
      ...
      if (context) {
        if (result.success) {
          ...
          this.updateStepStatus(context, 'crypto_transfer', 'failed', result.error);
        }
      }
      ...
      if (context) {
        this.updateStepStatus(context, 'crypto_transfer', 'failed', (error as Error).message);
      }
      return {
        success: false,
        error: (error as Error).message
   ...
   */
  private async createDexPayQuoteStep(transaction: OffRampTransaction): Promise<{ success: boolean; quoteId?: string; error?: string }> {
    const context = this.transactionContexts.get(transaction.id);
    if (context) {
    ...
    try {
      ...
      if (context) {
        this.updateStepStatus(context, 'quote_creation', 'failed', (error as Error).message);
      }
      return {
        success: false,
        error: (error as Error).message
   ...
   */
  private async finalizeTransactionStep(transaction: OffRampTransaction): Promise<{ success: boolean; orderId?: string; error?: string }> {
    const context = this.transactionContexts.get(transaction.id);
    if (context) {
    ...
    try {
      ...
      if (context) {
        if (result.success) {
          ...
          this.updateStepStatus(context, 'finalization', 'failed', result.error);
        }
      }

      const returnValue: { success: boolean; orderId?: string; error?: string } = {
      ...
      if (context) {
        this.updateStepStatus(context, 'finalization', 'failed', (error as Error).message);
      }
      return {
        success: false,
        error: (error as Error).message
   ...
   */
  private createTransactionContext(transaction: OffRampTransaction): TransactionContext {
   ...
   */
  private updateStepStatus(context: TransactionContext, stepName: string, status: TransactionStep['status'], error?: string): void {
   ...
   */
  private generateTransactionReceipt(transaction: OffRampTransaction): TransactionReceipt {
    const references: { crossmintTxId?: string; dexpayQuoteId?: string; dexpayOrderId?: string } = {};
   ...
   * Validate quote request
   */
  private validateQuoteRequest(request: QuoteRequest): { isValid: boolean; error?: string } {
    ...
    if (!request.bankCode || !request.accountNumber || !request.accountName) {
      return { isValid: false, error: 'Complete banking information is required' };
    }

    if (!request.fiatAmount && !request.tokenAmount) {
      return { isValid: false, error: 'Either fiat amount or token amount must be specified' };
    }
  ...
  private log(message: string): void {
    if (this.config.enableDetailedLogging) {
      console.log(`[TransactionManager] ${new Date().toISOString()}: ${message}`);
    }
  }
}

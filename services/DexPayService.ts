...
export class DexPayService implements IBankingManager, IDexPayService {
  ...
  async createQuote(quoteRequest: QuoteRequest): Promise<Quote> {
    try {
      ...
      logger.info(
        `Creating quote for workflow integration: ${quoteRequest.asset} on ${quoteRequest.chain}`,
      );
      ...
      logger.info(
        `Generated quote ${quoteData.id} for ${quoteRequest.asset} conversion in workflow context`,
      );
      ...
      logger.info(
        `Quote created successfully for workflow: ID=${quote.id}, Rate=${quote.rate}, Expires=${quote.expiresAt}`,
      );
      ...
      const errorMessage = this.getWorkflowFriendlyErrorMessage(
        error,
        "quote_creation",
      );
  ...
  async finalizeQuote(quoteId: string): Promise<QuoteResult> {
    try {
      logger.info(
        `Attempting to finalize quote ${quoteId} in workflow context`,
      );
      ...
      if (error.response?.status === 410) {
        logger.warn(
          `Quote ${quoteId} has expired during workflow finalization`,
        );
        return {
          success: false,
    ...
    try {
      ...
      if (resolution.isValid && resolution.accountName) {
        ...
        return {
          isValid: false,
          errors: [
            "Account not found. Please check the account number and bank selection.",
          ],
        };
      }
    } catch (error) {
  ...
  private validateQuoteRequest(quoteRequest: QuoteRequest): void {
    ...
    if (
      !quoteRequest.bankCode ||
      !quoteRequest.accountNumber ||
      !quoteRequest.accountName
    ) {
      throw new Error(
        "Complete banking information is required for quote creation",
      );
    }
    ...
    if (!this.isSupportedAssetChain(quoteRequest.asset, quoteRequest.chain)) {
      throw new Error(
        `Unsupported asset-chain combination: ${quoteRequest.asset} on ${quoteRequest.chain}`,
      );
    }
  }
    ...
    switch (context) {
      case "quote_creation":
        if (statusCode === 400) {
          ...
          if (
            errorMessage.toLowerCase().includes("asset") ||
  ...
  isQuoteExpired(quote: Quote): boolean {
    ...
    if (expired) {
      logger.info(
        `Quote ${quote.id} has expired at ${quote.expiresAt}, current time: ${now}`,
      );
    }
   ...
   */
  async regenerateExpiredQuote(
    originalQuoteRequest: QuoteRequest,
  ): Promise<Quote> {
    ...
    try {
      ...
      logger.info(
        `Successfully regenerated quote: ${newQuote.id} (expires: ${newQuote.expiresAt})`,
      );
      return newQuote;
    } catch (error) {
